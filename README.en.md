...
- **M33 — Log viewer GUI (`mcp-gui/`, Tauri 2.x + Vue 3)** (2026-09-27) — 81 new frontend cases (issue #25; GUI keeps its own version `0.1.0-beta.1`)
  - **Standalone read-only desktop app**: `mcp-gui/` is **fully decoupled** from the MCP server (it reads the filesystem only and does not need the server running) and unifies the four log types plus task artifacts in one UI. Business data is read-only end to end; the only writes are the app's own preferences (system config directory)
  - **Four log types**: `logs/server.log` (level/time-range filter + keyword highlighting), `task.jsonl` (separating state transitions / fine-grained agent events / the `note` progress channel; unparsable lines are skipped but counted), `agent-<round>.log` and `verify-<round>.log` (round switching, line numbers, wrapping), `report-<round>.{md,json,html}` and `dry-run-report-*` (Markdown rendering / structured cards / **sandbox iframe** visual preview / dry-run shown separately from regular reports / cross-round comparison)
  - **Large logs and live tail**: the first screen reads only a 64 KiB tail window and loads earlier chunks on demand showing "loaded N / total M"; `notify`-driven incremental refresh, **scrolling up pauses follow automatically**, with a one-click "Jump to latest"
  - **Cross-task search / export**: on-demand scanning (no local full-text index) with progress and cancellation; hits grouped as task → file → line with jump-to-hit; single-file export and whole-task zip (optionally excluding heavy raw logs)
  - **Dual-source auto-update**: **actual probing instead of system region** (correct behind a VPN) + a three-state switch + TTL cache + fallback to the last known good source; both manifests share the same version and signature, and **a failed minisign verification is always rejected**; update failures never affect log viewing
  - **CI isolation and gates**: a new standalone `GUI` workflow (windows / macos-15-intel / macos-15 matrix); `gui-v*` does not start with `v`, so it **never triggers** the MCP `release.yml`. A new **three-way vocabulary parity gate** (TS truth ↔ frontend mirror ↔ Rust mirror) fails the build on any drift. A **manual `workflow_dispatch` always builds** (an earlier revision silently skipped the whole matrix when the most recent commits happened not to touch `mcp-gui/`)
  - Usage and development docs in the [log viewer docs](docs/gui-log-viewer.en.md), hardware record in the [issue-25 record](docs/issue-25-gui-real-machine-record.md); the **GUI version evolves independently and is not released with the MCP package** (the package version is unaffected by this milestone)
