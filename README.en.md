...
- **M33 — Log viewer GUI (`mcp-gui/`, Tauri 2.x + Vue 3)** (2026-09-27) — 81 new frontend cases (issue #25; GUI keeps its own version `0.1.0-beta.1`)
  ...
  - **CI isolation and gates**: a new standalone `GUI` workflow (windows / macos-15-intel / macos-15 matrix); `gui-v*` does not start with `v`, so it **never triggers** the MCP `release.yml`. A new **three-way vocabulary parity gate** (TS truth ↔ frontend mirror ↔ Rust mirror) fails the build on any drift
