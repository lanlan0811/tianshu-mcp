...
npm ci && npm run typecheck && npm run lint && npm test && npm run build   # 期望 407/407 通过
```
...
- **macOS 未验证**：TraeWork 的原生对话框驱动（AppleScript 路线）、ZCode 真机闭环、Codex GUI 的就绪判定均未在
  macOS 实测；当前 macOS 分支一律 fail-closed（ZCode / Codex 内置 profile 在 darwin 上为 `research`）。
