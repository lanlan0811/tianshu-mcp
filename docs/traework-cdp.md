...
2. **新建会话**（点「新建任务」，每任务一个干净会话）
3. **绑定项目文件夹**：切到 Work 模式 → 点「选择文件夹（可选）」→ 在下拉里按项目名/路径匹配；
   未命中则点下拉底部「选择文件夹」→ 经受限 computer-use 驱动 Windows 原生对话框
4. **（可选）切换模型**：严格验证，不一致即报错，绝不静默用错模型
5. **写入任务书**（回读校验）→ **按 Enter 发送** → 轮询 DOM 直到出现完成标志
6. **自动验收**：命令检查（typecheck/lint/test/build，缺则跳过）+ 代码分析（相对 git 基线）
7. **验收失败**：自动生成修复计划文件 `rework-<taskId>-r<N>.md`，并把文件名写进同会话的返修消息
8. **再验收**，直到通过或轮次用尽（`needs_attention`）
