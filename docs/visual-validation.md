...
- **全量测试 486 passed / 10 skipped**（跳过的 10 项为真实浏览器门禁用例，已用 `TIANSHU_VISUAL_BROWSER_TEST=1` 单独跑通 10/10）：
  - 真实浏览器：托管浏览器启动与解码、三种页面来源、输入/点击/悬停/选项/滚动、整页懒加载、Cookie/localStorage 导入与失效诊断、外部资源允许与拦截、主文档 302 跳转来源校验、中文与带空格项目路径截图、截图差异与规则冻结。
...
- **macOS 13+ Intel 与 Apple Silicon** 的真实系统/Node/浏览器证据；Windows 10 的完整功能矩阵（端口冲突、就绪失败、取消清理、托管/本机浏览器与版本不匹配）。中文与带空格路径、外部资源跳转校验已在真实浏览器用例覆盖；CI 的 macOS runner 结果不能替代维护者在真实 macOS 设备上的验证记录。
