...
| 11 | 模型触发器误命中权限 chip（同组 4 个 chip 都带 `aria-haspopup="menu"`，仅模型 chip 无 `aria-label`） | 主选择器改 `button[aria-haspopup="menu"]:not([aria-label])` + 排除同组其它 chip |
| 12 | 思考强度实为 `role="slider"`（0–4 档）而非菜单项，原实现永远设不上 | 新增滑块选择器 + 方向键驱动（轻度/中/高/极高）；等级比较改精确匹配 |
| 13 | 绑定/新建后工具条重渲染，模型 chip 短暂空读被误判为「模型不符」 | 新增回读稳定等待（连续两次相同非空值） |
| 14 | 模型菜单需 **trusted** 点击才会展开，DOM `element.click()` 无效（报「无法打开 Codex 模型菜单」） | `click()` 统一改真实鼠标事件 + 元素可点等待重试 |
| 15 | 未登记项目走原生对话框路径不稳定（前台锁） | 新增 `registry.ts` 直接登记到 Codex 项目状态（幂等 + 备份 + 原子写 + 停实例时写），未登记项目实测全链路走通 |
| 16 | 登记需冷启动受管实例，60s 就绪超时不够（实测约 85s） | `launchTimeoutMs` 放宽到 150s |

## 4.1 第二轮：真实业务任务验收（2026-09-12）

用本 MCP 驱动 Codex 实际开发「切水果小游戏」（HTML+CSS+JS，`GPT-5.6 Sol` + 思考强度「高」，
项目 `D:\Trae项目\切水果小游戏`），32 分钟后 Codex 产出 `index.html` / `style.css` / `game.js`，
验收标准全部通过，并在无头 Edge 中实测可玩：

- 点击「开始切水果」→ 游戏启动（开始页消失）；
- 连续滑动 → **得分上升**（实测到 2、6 分）且**生命按漏接扣减**；
- 生命耗尽 → Game Over 显示最终得分/最佳连击 → 「再来一局」可重开（实测循环 5 次）；
- 全程无 JS 异常；`game.js` 语法校验通过；无任何外部资源引用（favicon 为 `data:` 内联）。

该轮在真机上又暴露并修复 3 个缺陷（详见 §4 表 11–13）：模型触发器误命中权限 chip、
思考强度实为滑块而非菜单项、触发器重渲染期间空读。另说明：该项目的 Codex 侧项目登记
（`local_projects`）由本测试预置；未登记的新项目仍走原生对话框路径（见 §5）。
