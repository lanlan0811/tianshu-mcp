...
- **npm publish**: `tianshu-mcp@0.5.0` has been published to npm `latest` (confirmed by a direct registry query with a `dist.shasum` matching the local build; a fresh isolated install passes all four `visual doctor` checks). Plan §7 did not originally include npm publishing; it was performed on the user's explicit instruction to keep the "published continuously since v0.1.1" continuity intact.
