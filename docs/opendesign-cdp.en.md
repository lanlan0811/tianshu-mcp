...
### Selector-independent modules (P2/P5/P6 delivered ahead of schedule)

| File | Contents | Why it can go first |
|---|---|---|
| `workspace.ts` | Working-directory binding orchestration: skip when already bound → expand the trigger → click "Select directory" → native dialog → **read-back verification**; `normalizeWorkspacePath` / `workspaceMatches` (including prefix matching for UI-truncated ellipsis values) | The orchestration and judging logic depend only on semantic keys, not on concrete CSS |
| `dialog.ts` | `toNativeDialogPath` (absolutise + upper-case drive + backslashes), `listOwnedDialogs`, `closeStrayDialogs`, `selectOpenDesignFolder` (**two routes**: WM_SETTEXT first, keyboard input as fallback; both require a matching read-back, and success requires the dialog to **actually close**) | The native dialog is a Win32 surface, independent of page DOM |
| `liveness.ts` | **Three-signal** judging: stop-button visibility + conversation-text hash + **artifact mtime/size fingerprint**; pure function `judgeOpenDesignPoll` | Inputs are collected snapshots; judging itself is selector-independent |
| `fixplan.ts` | Repair/optimisation plans written to the **project root** `.opendesign/plans/opendesign-fix-r<N>.md` (per-round, never overwritten) + fix-prompt assembly | Plans are generated by MCP, independent of page actions |
