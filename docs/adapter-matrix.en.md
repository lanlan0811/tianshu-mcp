...
| **TraeWork / TRAE SOLO CN** | desktop IDE (measured 2026-09-23 registry `0.1.64` / file version `2.3.82597`; the earlier v1.107.1 note was a stale string) + **CDP GUI driver** | ✅ **integrated & machine-verified** (2026-09-08; see T1 correction and [traework-cdp.en.md](traework-cdp.en.md)); install discovery completed 2026-09-23 with `discovery.ts` (issue #23) | reuses TraeWork desktop login (this MCP reads no credentials) | no headless CLI; drives the chat UI via `--remote-debugging-port`; discovery order explicit → fixed-drive rel → registry `InstallLocation` → shortcut → standard dirs → PATH; on Windows only `TRAE SOLO CN.exe` is accepted | replies extracted from the DOM; project files written by TraeWork itself; when the **port is not ready** it prints diagnostics (child exit code / port listeners / existing instances) — diagnosis only, no launch-strategy change |
