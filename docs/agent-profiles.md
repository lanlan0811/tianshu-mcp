...
> `verifyCommandTimeoutMs`、`verifyConcurrency`、`skills.autoInstall`（`true` \| `"prompt"` \| `false`）、
> `skills.backupKeep`（默认 3，覆盖后保留的历史技能备份个数，`0` = 不清理），以及 **`shutdown.guiStopWaitMs`（默认 15000）**——
