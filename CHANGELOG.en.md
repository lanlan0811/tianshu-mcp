...
- **53** new cases: `opendesign-workspace.test.ts` ×14 (path comparison, skip-when-bound with zero clicks, the success path and **baseline sampling order**, the four failure paths of missing trigger / missing menu item / native failure / read-back mismatch, and empty read-back), `opendesign-liveness.test.ts` ×20 and `opendesign-dialog-fixplan.test.ts` ×19; `opendesign-dom.test.ts` gained an override-semantics regression.
- Full suite: **1256 passed / 12 skipped** (109 files); `typecheck`, `eslint src test scripts` and `build` all green.
